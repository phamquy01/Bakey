                ...
                <div class="form-group">
                    ...
                    <div class="checkbox mt-2" style="height: 550px; overflow-y: auto">
                        @foreach($allRoute as $key =>$route)
                            <div class="routeItem">
                            <input type="checkbox" name="route[]" id="route-{{$key}}" value="{{$route}}" {{in_array($route, $routes) ? "checked" : ""}}>
                            <label for="route-{{$key}}">{{$route}}</label>
                            </div>
                        @endforeach
                    </div>
                </div>
                <button type="submit" name="btnCreate" value="Thêm mới" class="btn btn-primary">Cập nhật</button>
